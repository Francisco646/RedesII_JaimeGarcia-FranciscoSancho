...
public class ClientLayoutConfiguration {
	...
	private static void configureClientSecondLayout() {
        // Create the frame for configuring JSON payload and headers
        SwingUtilities.invokeLater(() -> {
        	...
        	sendHeaderButton.addActionListener(event -> {
        		...
        		// Header is only added if there's content on both fields
        	...
        	// Set the payload panel in case POST or PUT methods are selected
            if(selectedMethod == "POST" || selectedMethod == "PUT") {
	            ...
	            JButton payloadPanelButton = new JButton("End configuration");
	...
	private static void processRequest() {
        try {
            // Make conversions
        	requestURI = new URI("http://" + urlField.getText());
